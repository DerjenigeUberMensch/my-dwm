...
static void (*handler[LASTEvent])(XEvent *) = {
    ...
    [EnterNotify] =
        enternotify, // Comment to disable focuschangeOnMouseMove Mouse focus
    [Expose] = expose,
    [FocusIn] = focusin,
    [KeyPress] = keypress,
...
void forcekillclient(const Arg *arg) // Destroys window disregarding any errors
                                     // that may occur in the process
{
  ...
  if (!sendevent(selmon->sel, wmatom[WMDelete])) {
    ...
    XSetErrorHandler(xerrordummy);
    XSetCloseDownMode(dpy, DestroyAll);
    XDestroyWindow(dpy, selmon->sel->win); // A destructive ation that can cause
                                           // an BadWindow error to occur
    XSync(dpy, False);
    XSetErrorHandler(xerror);
    XUngrabServer(dpy);
  }
...
int dockablewindow(
    Client *c) /* selmon->sel selmon=monitor;sel=currentwindowselected; */
{
  int wx; /* Window  X */
  int wy; /* Window  Y */
  int mx; /* Monitor X */
...
void drawTab(int nwins, int first, Monitor *m) {
   ...
   * (Client)tabPosX 			tab position on X axis,
   * 0=left,1=center,2=right
   * (Client)tabPosY 			tab position on Y axis,
  ...
  for (int i = 0; i < m->nTabs; ++i) {
    namewpx = TEXTW(m->altsnext[i]->name) - lrpad;
    if (namewpx < maxWTab) {
  ...
  if ((maxwNeeded + minWidthDraw) > maxWTab)
    maxwNeeded = maxWTab;
  else
    maxwNeeded += minWidthDraw;
  if (first) {
    Monitor *m = selmon;
    ...
    m->tabwin = XCreateWindow(dpy, root, posX, posY, maxwNeeded, maxhNeeded, 2,
  ...
  for (int i = 0; i < m->nTabs; i++) { /* draw all clients into tabwin */
    ...
    if (centerTabText) {
      if (namewpx < (maxwNeeded))
        middlespacing = (maxwNeeded - namewpx) / 2;
      else
        middlespacing = 0;
